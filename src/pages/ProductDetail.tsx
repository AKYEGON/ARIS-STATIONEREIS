...
const ProductDetail = () => {
  ...
  if (notFound || !product) {
    return (
      <div className="min-h-screen flex flex-col">
        <SEO title="Product Not Found" description="The product you are looking for could not be found." canonicalUrl={`/product/${slug}`} noindex />
        <Header />
  ...
  const fullImage = product.image.startsWith("http") ? product.image : `https://arisstationaries.co.ke${product.image}`;

  const seoTitle = `Buy ${product.name} in Kenya — KSh ${displayPrice.toFixed(0)} | Aris Stationeries`.slice(0, 70);
  const seoDescription = `${product.name} available at Aris Stationeries Kenya for KSh ${displayPrice.toFixed(0)}. ${product.description || "Affordable stationery"} — delivered to your university or doorstep across Kenya.`.slice(0, 160);
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <main className="flex-1 container mx-auto px-3 sm:px-4 py-4 sm:py-6 pb-20">
        {/* Breadcrumbs */}
        <nav aria-label="Breadcrumb" className="flex items-center gap-1 text-xs sm:text-sm text-muted-foreground mb-4 overflow-x-auto whitespace-nowrap">
        ...
        <div className="grid md:grid-cols-2 gap-6 lg:gap-10 mb-10">
                ...
                {allMedia.map((m, i) => (
                  <button
                    ...
                    className={`flex-shrink-0 w-16 h-16 rounded-md border-2 bg-white overflow-hidden transition-all ${
                      i === activeMediaIndex ? "border-primary" : "border-border opacity-70 hover:opacity-100"
                    }`}
                  >
                    {m.type === "image" ? (
                      <img src={m.url} alt={`${product.name} ${i + 1}`} className="w-full h-full object-contain p-1" />
                    ) : (
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-baseline gap-3">
              <span className="text-2xl sm:text-3xl font-bold text-primary">KSh {displayPrice.toFixed(0)}</span>
              ...
              Object.entries(variantGroups).map(([type, variants]) => (
                <div key={type}>
                  <p className="text-sm font-semibold mb-2">
                    {type} {!selectedVariant && <span className="text-destructive font-normal">*required</span>}
                  </p>
                  <div className="flex flex-wrap gap-2">
                    {variants.map((v) => (
                      ...
                      >
                        {v.variant_value}
                        <span className="ml-1.5 opacity-70 text-xs">KSh {v.price.toFixed(0)}</span>
                      </button>
                    ))}
                  </div>
                </div>
            ...
            <div className="grid grid-cols-3 gap-2 pt-3 border-t">
              <div className="flex flex-col items-center text-center gap-1 p-2">
                ...
                <span className="text-[10px] sm:text-xs text-muted-foreground">Nationwide Delivery</span>
              </div>
              <div className="flex flex-col items-center text-center gap-1 p-2">
                <ShieldCheck className="h-5 w-5 text-primary" />
                <span className="text-[10px] sm:text-xs text-muted-foreground">Genuine Stationery</span>
              </div>
              <div className="flex flex-col items-center text-center gap-1 p-2">
                <Phone className="h-5 w-5 text-primary" />
                <a href="tel:+254119774470" className="text-[10px] sm:text-xs text-muted-foreground hover:text-primary">
                  +254 119 774 470
                </a>
              </div>
            </div>
          ...
          <section className="pt-6 border-t">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
              {related.slice(0, 8).map((p) => (
                <ProductCard key={p.id} product={p} onAddToCart={(prod, v) => addToCart(prod, v)} />
              ))}
            </div>
          </section>
        )}
